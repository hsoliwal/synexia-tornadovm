...
/**
 * Compiled basic-block execution tier.
 *
 * <p>Each accelerator lane looks up the block for its current PC and executes the packed
 * micro-op stream directly. A miss sets {@code fallbackMask[core]} so the architectural
 * interpreter task can handle only that core in the same execution plan.
 */
public final class RiscV32BlockKernel {

    private RiscV32BlockKernel() {
    }
    ...
    public static void runOneBlock(IntArray registers, IntArray pc, IntArray status, IntArray trapCause,
        ...
        for (@Parallel int core = 0; core < coreCount; core++) {
            ...
            // The full interpreter owns asynchronous interrupt entry. Do not execute a hot block
            // ahead of an interrupt that is architecturally pending at this instruction boundary.
            ...
            if (operationCount <= 0 || operationCount > instructionBudget) {
                fallbackMask.set(core, (byte) 1);
                continue;
            }
            ...
            for (int index = 0; index < operationCount && !stopBlock
                ...
                switch (kind) {
                    ...
                    case RiscV32MicroOp.SW: {
                        ...
                        int source2 = readRegister(registers, coreCount, core, rs2);
                        int address = source1 + immediate;
                        int width = kind == RiscV32MicroOp.SB ? 1 : kind == RiscV32MicroOp.SH ? 2 : 4;
                    ...
                    default:
                        // Should be impossible for compiler-produced blocks. Preserve correctness
                        // if metadata is corrupt by handing the PC to the interpreter next quantum.
                        fallbackMask.set(core, (byte) 1);
                        stopBlock = true;
                        continue;
                }
            ...
            compiledBlockExecutions.set(core, compiledBlockExecutions.get(core) + 1);
        }
    }
